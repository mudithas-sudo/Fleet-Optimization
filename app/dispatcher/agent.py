...
dispatch_agent = Agent(
    ...
    instruction=(
        ...
        "deadlineInMinutes) go out first, on idle drivers.\n"
        "3. VERIFY every batch you intend to propose with evaluate_route "
        "before including it. Its ETAs are the only valid timing source - "
        "never estimate travel times yourself. If a parcel comes back atRisk, "
        "try a different grouping, ordering or driver; if nothing works, "
        "still assign it but explain the risk in riskNotes, or leave it "
        "unassigned with the reason if the deadline is already hopeless.\n"
